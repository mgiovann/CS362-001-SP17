	...
	//			System.out.println(" Seed:"+randomseed );
    		...
    		for (int i = 0; i < NUM_TESTS; i++) {
    			...
    			if (methodName.equals("setStartHour")){
    				int newHour = ValuesGenerator.getRandomIntBetween(random, -30, 30);
    				appt.setStartHour(newHour);
    			}
    			if (methodName.equals("setStartMinute")){
    				int newHour = ValuesGenerator.getRandomIntBetween(random, -100, 100);
    				appt.setStartMinute(newHour);
    			}
    			if (methodName.equals("setStartDay")){
    				int newHour = ValuesGenerator.getRandomIntBetween(random, -50, 500);
    				appt.setStartDay(newHour);
    			}
    			if (methodName.equals("setStartMonth")){
    				int newHour = ValuesGenerator.getRandomIntBetween(random, -5, 20);
    				appt.setStartMonth(newHour);
    			}
    			if (methodName.equals("setStartYear")){
    				int newHour = ValuesGenerator.getRandomIntBetween(random, -1, 3004);
    				appt.setStartYear(newHour);
    			}
